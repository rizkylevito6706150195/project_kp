...
if(empty($data1) or empty($data2) or empty($data3) or empty($jawaban1) or empty($jawaban2) or empty($jawaban3) or empty($jawaban4) or empty($jawaban5) or empty($jawaban6) or empty($jawaban7)){
	...
	header("location:../mentee_form4.php?pesan=error1");
	die;
}


$sql = "INSERT INTO data_form_mentee (username, nomor_form, jawaban1, jawaban2, jawaban3, jawaban4, jawaban5, jawaban6, jawaban7, jawaban8, jawaban9, jawaban10) 
		VALUES ('$username', '4', '$data1', '$data2' ,'$data3' ,'$jawaban1', '$jawaban2', '$jawaban3', '$jawaban4', '$jawaban5', '$jawaban6', '$jawaban7')";
$query = mysqli_query($conn, $sql);

$sql2 = "INSERT INTO history_mentee (nomor_form, judul_form, status, tanggal, username) 
		VALUES ('4', 'Evaluasi Mentee', 'selesai', '$tanggal', '$username')";
